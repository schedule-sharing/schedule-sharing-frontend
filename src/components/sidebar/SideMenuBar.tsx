    ...
    <div className={classes.root}>
      ...
      <Drawer
        ...
        <List className={classes.list}>
          {clubs.clubs.map((club: clubType) => (
            <ListItem
              ...
              style={club.clubId === clubs.currentClub.clubId ? { backgroundColor: "orangered" } : { backgroundColor: "inherit" }}
              onClick={() => selectClub(club.clubId as string)}
              button>
              <ListItemText>
                <Typography align="center" children={club.clubName} variant="h6" />
              </ListItemText>
            </ListItem>
          ))}
        </List>
        ...
        <Grid container justify="center">
          <IconButton onClick={handleClubFormVisibility} classes={{ root: classes.addIcon }}>
            <Add fontSize="large" />
          </IconButton>
        </Grid>
      </Drawer>
      <main className={classes.content}>{children}</main>
      <ClubForm visibility={clubForm} setVisibility={handleClubFormVisibility} />
    </div>
  );
};
